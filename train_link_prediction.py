...
from utils.linkdata_processing import get_data, compute_time_statistics
...
def build_common_parser():
  ...
  parser.add_argument('--config', type=str, default='configs/train_link_prediction_uci.json',
                      help='Path to a JSON config file. Config values override parser defaults.')
  parser.add_argument('-d', '--data', type=str, choices=['uci', 'UCI', 'wikipedia', 'reddit', 'enron'],
                      help='Dataset name (eg. wikipedia or reddit)', default='uci')
  ...
  parser.add_argument('--train_num_batch', type=int, default=None,
                      help='Number of training batches per epoch. Defaults to the dataset setting.')
  return parser
...
TRAIN_NUM_BATCH = args.train_num_batch if args.train_num_batch is not None else dataset_spec['default_train_num_batch']
PREFIX = args.prefix if args.prefix is not None else f"{dataset_spec['default_prefix']}_{args.embedding_module}_l{args.n_layer}"
RUN_NAME = f"{PREFIX}_{args.memory_updater}_{args.aggregator}_{args.message_function}"

Path("./saved_models/").mkdir(parents=True, exist_ok=True)
Path("./saved_checkpoints/").mkdir(parents=True, exist_ok=True)
MODEL_SAVE_PATH = f'./saved_models/{RUN_NAME}-tgn-link-prediction.pth'
get_checkpoint_path = lambda \
    epoch: f'./saved_checkpoints/{RUN_NAME}-{epoch}-link-prediction.pth'
...
logger.info(args)
logger.info(f"Resolved prefix: {PREFIX}")
logger.info(f"Resolved run name: {RUN_NAME}")
logger.info(f"Message dimension: {MESSAGE_DIM}")
logger.info(f"Memory dimension: {MEMORY_DIM}")
logger.info(f"Train num batch: {TRAIN_NUM_BATCH}")
...
for i in range(args.n_runs):
  results_path = "results/{}_{}.pkl".format(RUN_NAME, i) if i > 0 else "results/{}.pkl".format(RUN_NAME)
  ...
  train_num_batch = min(TRAIN_NUM_BATCH, num_batch)
  logger.info('train batches used per epoch: {}'.format(train_num_batch))
  ...
  for epoch in range(NUM_EPOCH):
    ...
    for k in range(0, train_num_batch, args.backprop_every):
      # print('batch',k)
      loss = 0
      optimizer.zero_grad()
      ...
      for j in range(args.backprop_every):
        ...
        pos_prob = pos_prob.float()
        neg_prob = neg_prob.float()
