...
class Memory(nn.Module):
  ...
  def backup_memory(self):
    ...
    for k, v in self.messages.items():
      messages_clone[k] = [(x[0].clone(), x[1].clone(),x[2].copy()) for x in v]
  ...
  def restore_memory(self, memory_backup):
    ...
    for k, v in memory_backup[2].items():
      self.messages[k] = [(x[0].clone(), x[1].clone(),x[2].copy()) for x in v]
  ...
  def detach_memory(self):
    ...
    for k, v in self.messages.items():
      new_node_messages = []
      for message in v:
        new_node_messages.append((message[0].detach(), message[1],message[2]))
